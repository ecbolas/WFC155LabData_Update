#### Welcome to your third lab for WFC 155! ####
...
#   data in R.
# We will work in the sf package, which is a spatial package that integrates with the 
#     tidyverse. 
# All of our animal movement track analysis will be conducted using the amt package.

## Lab 3 OBJECTIVES ##
# 1. Load spatial data in R and use R as a GIS (geographic information system)
...
# We need to install a few more packages for this week if you are on a non-lab computer:
...
# You may want to include it in a WFC 155 folder (if you have one). I'll call mine 
#   "WFC155_labs"
...
tsavo_lions <- read_csv("Tsavo_Lions.csv")

# Alternatively, we can create an R project.
# By working in an R project, you only need to direct R to subfolders within your
#   R project folder when loading data.

# This makes it easy to move the parent folder containing your R project, scripts, 
#   and data to a new computer without having to go through and update the
#   working directory in all your scripts. It also makes it easier to share work
#   with collaborators.

# Once you've created your "WFC-155_labs" parent folder, return to RStudio and navigate
#   to File > New Project > Existing Directory.
#   Enter the file path for your parent folder - mine is 
#   "C:/Users/justinesmith/WFC155/WFC155_labs" - then click "Create Project"

# If you return to your "WFC-155_labs" folder you should now see a new R Project file.
#   You can include "data", "scripts", and "figures" subfolders to keep everything organized.

# Move Tsavo_Lions.csv to your data folder. Now when you work in your R project
#   all you need to do is reference the subfolder and data file name when loading data.
#   You won't have to go through the hassle of checking and setting your working directory each time!

# This is all you need to do when loading data from an R project.
...
##** QUESTION 2: Between Diana and Romeo, which traveled further to the west?
...
Romeo_track <- make_track(tbl = tsavo_Romeo, .x = UTMeasting, .y = UTMnorthing, .t = timestamp_local, 
                        crs = myCRSutm)
...
Romeo_track %>% 
  track_resample(rate = hours(7), tolerance = minutes(15))  -> Romeo_track

# How do your sampling rates look now? 
#** QUESTION 3. How has the minimum value changed? What about the number of locations (n) -
...
Romeo_track %>% 
  filter_min_n_burst(min_n = 2) -> Romeo_track
...
Romeo_steps  %>% 
  time_of_day(where = "start") -> Romeo_steps
...
ggplot(Romeo_steps, aes(x = sl_, fill = tod_start_)) +
  geom_density(alpha = 0.4) +
  scale_fill_brewer(palette = "Dark2")

#** QUESTION 4: By looking at your figure, does Romeo generally move more than 5 kilometers
...
#** QUESTION 5: Copy and paste your pipe (steps 4 & 5 above) into your lab report
...
#** QUESTION 6: Does Diana move farther on average during the day or at night? What is her 
...
#** QUESTION 7: Based on your turn angle plot, does Diana or Romeo have more movements
#     that look like "directed travel"? Explain how you arrived at your answer. Please include 
#     your plot in your lab report!

#--------------#
#### QUESTIONS FROM LECTURE AND READINGS ####

#** QUESTION 8: Did Suraci et al. 2019 find that African lions selected habitats based on distance to 
#         boma differently depending on their behavioral state and time of day? Describe similarities and 
#         differences in the response to bomas when separated by behavioral state and time of day.
