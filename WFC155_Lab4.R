#### Welcome to your fourth lab for WFC 155! ####
...
# We need to install a few more packages for this week, if outside of lab:
...
elevation <- raster("sgnp_DEM.tif")
NDVI <- raster("sgnp_NDVI.tif")
habitat_class <- raster("sgnp_habitat.tif")
...
border <- readOGR("sgnp_border.shp")
roads <- readOGR("sgnp_roads.shp")
...
#** QUESTION 1: Where is the highest elevation area inside the park? Where is the lowest
...
#   pumas in the park.
#** QUESTION 2: About how far away, to the nearest kilometer, is the random point that is 
#     closest to that meadow? Make an estimate by looking tat the scale bar in the bottom
#     left of the map.
...
#* QUESTION 3: What is the "mode" (i.e. highest frequency) of elevations in our sample? Think
...
dist_to_road_matrix <- gDistance(available_points,roads, byid=TRUE)
...
#* QUESTION 4: Make a histogram of distance to road. Are most random locations within 5 km 
...
# For example, we can hypothesize that distance to road is function of slope in the park. We might
#   predict that dist to road decreases as slope increases if we think that steep slopes are hard  
#   to build on. Let's  check that relationship out using our ggplot 
...
#   where distance to road increases (as predicted) with slope. What do you think is going on at
#   the points close to a road with very steep slopes?

# Your turn!
#* QUESTION 5: Make a hypothesis and prediction about which covariate (aside from habitat class)
#   might predict NDVI at our random locations. In your prediction, 
...
# "1", "2", and "3" are not super intuitive! Luckily, we can rename the levels to be more descriptive
...
# Your turn!
#* QUESTION 6: Just like in Question 5, write a hypothesis, prediction, and justification 
#   about a covariate that you think would differ across habitat types. Make a boxplot to 
...
#** QUESTION 7: What are the differences in how generalist vs specialist populations respond 
#     to habitat loss when considering patch occupancy?

#** QUESTION 8: How do beavers affect wildfire risk in riparian zones?
