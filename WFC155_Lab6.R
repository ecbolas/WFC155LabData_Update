...
# New packages for today if outside lab
...
coyote_available_sp <- as_Spatial(coyote_available)
coyote_used_sp <- as_Spatial(coyote_used)
...
levelplot(crop(utah_NLCD,extent(coyote_available)), col.regions=c(landcoverkey$colortable)) +
  latticeExtra::layer(sp.points(coyote_used_sp,col = alpha("black",0.3),pch = 20))

# Your turn!
#   Make a similar plot to the one above but with both used and available locations (color "purple")
#** QUESTION 2: Name one habitat type that you think has more available points than used,
...
#   Hint: revisit the barplot code in lab 2!
...
#** QUESTION 3: Which two habitat types are most available in coyote C028's home range? Which
...
coyote_sum %>%
  pivot_wider(names_from = Used, values_from = c(count,proportion)) -> coyote_sum_2

# Now we have a dataframe of 8 habitat types with their proportion of available and used points
coyote_sum_2

# Your turn!
# Add a column to coyote_sum_2 using mutate() of the calculated SRs. 
#     Name the column selection_ratio
#     Name the new dataframe coyote_sum_3
...
ggplot(coyote_sum_3,aes(x = NLCD,y = selection_ratio,fill = selection_ratio)) +
  geom_bar(stat = "identity") + 
  theme(axis.text.x = element_text(angle = 50, hjust = 1)) +
  geom_hline(yintercept = 1, linetype="dashed")

#** QUESTION 4: Which habitats were selected for? Which would you say seems to be the most 
...
# To start you off, I've provided the function to isolate F53's home range
...
#       ONE DIFFERENCE FROM ABOVE CODE: instead of using rbinom to randomly assign day or
#         night, you should assign them explicitly. So, for your st_sample code for day,
#         replace the "rbinom" code with simply "daynight = 1", and for the night dataset
#         write "daynight = 0".
...
#** QUESTION 5: Look at your plots from step 7. Do you notice any differences in proportion
#     of used habitats between day and night? If so, what are they? Are any habitats used in 
#     at night but not during the day?
# Export both plots for your lab report

#** QUESTION 6: Look at your plots from step 9. Which habitat(s) were selected for at night
...
#** QUESTION 8: Name one way in which the exportation of the Yellowstone Model has  
#     impacted local people who live abroad near protected areas.
