#### Welcome to your fifth lab for WFC 155! ####
...
# We need to install one more package for this week if outside of lab:
...
alberta_wolves <- read_csv("Alberta_Wolf.csv") 
...
#   (level) of the animal locations inside it. Generally, ecologists use a 95% level. That
#   means, the MCP of the animal is the smallest polygon that still contains 95% of the 
#   animal's locations. MCPs also can't have any inward-facing (concave) angles along the 
#   outside - all lines connecting points around the perimeter have to point out (convex)
...
#     square kilometers.
wolf32253_mcp_area <- mcp.area(wolf32253[1], unin = c("m"), unout = c("km2"))
...
wolf32253_mcp_100 <- mcp(wolf32253[1], percent=100)
wolf32253_mcp_95 <- mcp(wolf32253[1], percent=95)
wolf32253_mcp_50 <- mcp(wolf32253[1], percent=50)
...
ggplot() + 
  ...
  geom_sf(data = st_as_sf(wolf32253),cex = 0.8, pch = 21, col = "black", 
          fill = "yellow", alpha = 0.5)

#** QUESTION 1: Now that you can see why the 100% level is so much larger than the 95% level,
...
wolf32253_kud_href <- kernelUD(wolf32253[1], h = "href")
...
# YOUR TURN!
# Make a plot showing the wolf locations (the wolf GPS points) and the 95% and 50% KUDs for 
#       wolf 32253.
#   Feel free to change the colors or other graphical specifications (cex, pch, alpha) if 
#     you want to improve your visualization!
#** QUESTION 2: Let's say we are trying to preserve this wolf's home range, but we only have
...
#** QUESTION 3: For this individual wolf, is the MCP or KUD home range generally larger across
...
#** QUESTION 4: Export the plot for your lab report.
...
#** QUESTION 5: Looking at this plot and your plot from QUESTION 4, describe a scenario in 
#     which you might calculate a home range with a large bandwidth rather than a small 
#     bandwidth. Explain your reasoning.
...
# --> If you get an error that your plot margins are too large, enter "par(mar=c(1,1,1,1))"
#     without the quotes into the console and then try to plot again. This function lets
#     you set the margins around your plots manually.
# MCP:
...
#** QUESTION 6: Which two individuals appear to be in the same wolf pack (they use the
#     same area)?
...
#** QUESTION 7: Which individual overlaps the most oil and gas extraction in their home range?

#--------------#
#### QUESTIONS FROM LECTURE AND READINGS ####

#** QUESTION 8: Give an example of a prey behavior that can cause a behaviorally mediated
#       trophic cascade, and briefly explain how.

#** QUESTION 9: Why is it hard to evaluate if wolves caused a trophic cascade in 
#       Yellowstone when they were reintroduced?
